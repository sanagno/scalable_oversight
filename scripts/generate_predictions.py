...
from src.definitions import (ADDITIONAL_SYSTEM_PROMPTS, FIELDS,
                             HAS_DATASET_EXPLANATIONS, LEVELS,
                             POSSIBLE_ADVOCATES, RESPONSE_TYPES,
                             SYSTEM_PROMPTS)
...
if __name__ == "__main__":
    ...
    system_prompts = {
        k: v.format(field=FIELDS[args.dataset])
    ...
    for system_prompt_name, system_prompt in system_prompts.items():
        ...
        for advocate_level, include_explanation in POSSIBLE_ADVOCATES:
            if args.only_assistant and (
                advocate_level != "assistant" and advocate_level != "None"
            ):
                continue
            ...
            dataset, choices = get_dataset(
                ...
                if advocate_level not in LEVELS
                    ...
                    args.advocate_data_folder,
                    f"generations_{advocate_level}.pkl",
                ),
            ...
            logger.info(
                f"System prompt {system_prompt_name} advocate level"
                f" {advocate_level} include_explanation"
                f" {include_explanation} metric is: {metrics}"
            )
